...
def create_lab_initial_parameters(layer_dims: list[int], seed: int = 42, scale: float = 0.01) -> dict[str, np.ndarray]:
    rng = np.random.default_rng(seed)
    parameters: dict[str, np.ndarray] = {}
    for layer in range(1, len(layer_dims)):
        input_dim = layer_dims[layer - 1]
...
class NumpyMLPClassifier:
    ...
    def initialize_parameters(self) -> dict[str, np.ndarray]:
        if self.initialization == "lab":
            return create_lab_initial_parameters(self.layer_dims, seed=self.seed, scale=0.01)
    ...
    def set_parameters(self, parameters: dict[str, np.ndarray]) -> None:
        self.parameters = {name: value.copy() for name, value in parameters.items()}
