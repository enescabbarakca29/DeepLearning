...
def train_torch_model(
    ...
    if initial_parameters is not None:
        with torch.no_grad():
            model.hidden.weight.copy_(torch.tensor(initial_parameters["W1"], dtype=torch.float32))
            model.hidden.bias.copy_(torch.tensor(initial_parameters["b1"].reshape(-1), dtype=torch.float32))
            model.output.weight.copy_(torch.tensor(initial_parameters["W2"], dtype=torch.float32))
            model.output.bias.copy_(torch.tensor(initial_parameters["b2"].reshape(-1), dtype=torch.float32))
